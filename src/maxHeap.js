...
class MaxHeap {
  ...
  remove() {
    ...
    while (needsRebalancing) {
      ...
      if (rightChild && lastElt.priority < rightChild.priority && (!leftChild || leftChild.priority < rightChild.priority)) {
        this._swap(rightChildIdx, lastEltIdx);
        lastEltIdx = rightChildIdx;
      } else if (leftChild && lastElt.priority < leftChild.priority && (!rightChild || rightChild.priority < leftChild.priority)) {
